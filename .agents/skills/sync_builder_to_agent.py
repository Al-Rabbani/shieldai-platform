#!/usr/bin/env python3
"""
sync_builder_to_agent — PEA dual-DB sync skill v1.1
...
UA          = "Mozilla/5.0 (compatible; PEA-Sync/1.1)"
...
def main():
    ...
    SKIP = ["invalid", "probe", "pea.e2e", "pea.live", "pea.schema",
            "logtest", "timing.probe", "test-builder", "test-agent", "schema.test"]

    prod = [a for a in builder_recs
            if a.get("reference_code")
            and not any(k in (a.get("applicant_email") or "").lower() for k in SKIP)
            and not any(k in (a.get("reference_code") or "").lower() for k in SKIP)]
    ...
    for a in prod:
        ...
        if ref not in agent_index:
            ...
            print(f"  CREATED {ref} → id={result.get('id','?')}")
            print(f"    {flat.get('applicant_name')} | {flat.get('venture_name')} | nat={flat.get('nationality')} | phone={flat.get('phone_number')} | stripe={bool(flat.get('stripe_session_id'))}")
            created += 1
        else:
            ex = agent_index[ref]
            needs = (
            ...
            if needs:
                patch = {k: flat[k] for k in [
                    ...
                    "venture_name","venture_stage","venture_sector","date_of_birth","applicant_role"
    ...
    print(f"\nSync done — created={created} updated={updated} skipped={skipped} | total builder={len(prod)}")
