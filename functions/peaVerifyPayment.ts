/**
 * peaVerifyPayment — NEW
 * Called by the /payment-success page with ?session_id=cs_live_xxx
 * Verifies the Stripe session is actually paid, updates Application status,
 * starts the 90-day clock, creates a PaymentTransaction record, and sends
...
function buildPaymentConfirmEmail(app: Record<string, any>, session: Record<string, any>, year: number): string {
...
<div style="max-width:600px;margin:0 auto;">
  <div style="background:#111827;border-bottom:3px solid #22c55e;padding:32px 40px;text-align:center;">
    ...
    <div style="color:#22c55e;font-size:14px;letter-spacing:2px;margin-top:8px;">Payment Confirmed — Review Has Begun</div>
  </div>
  <div style="padding:32px 40px;background:#111827;">
    ...
    <p style="color:#94a3b8;font-size:13px;line-height:1.8;margin-bottom:22px;">
      Your endorsement fee has been successfully processed. Your 90-day expert review period has officially begun.
    </p>
    ...
    <table style="width:100%;border-collapse:collapse;background:#0d1220;border:1px solid #1e293b;border-radius:6px;margin-bottom:20px;">
      ...
      <tr><td style="padding:10px 14px;color:#C9A84C;font-size:14px;font-weight:700;">Total Paid</td><td style="padding:10px 14px;color:#C9A84C;font-size:14px;font-weight:700;">£1,200.00</td></tr>
      <tr><td style="padding:10px 14px;color:#94a3b8;font-size:12px;border-top:1px solid #1e293b;">Stripe Session</td><td style="padding:10px 14px;color:#475569;font-size:11px;font-family:monospace;border-top:1px solid #1e293b;">${session.id || "—"}</td></tr>
    ...
    <div style="background:#0a1a0a;border:1px solid #166534;border-radius:6px;padding:16px 18px;margin-bottom:20px;">
      <div style="color:#22c55e;font-size:12px;font-weight:700;letter-spacing:2px;text-transform:uppercase;margin-bottom:10px;">90-Day Review Timeline</div>
      <div style="color:#94a3b8;font-size:13px;line-height:1.8;">
        📅 <strong style="color:#e2e8f0">Day 0:</strong> Payment Confirmed — Review Begins<br/>
        📋 <strong style="color:#e2e8f0">Day 30:</strong> First Panel Update<br/>
        🔍 <strong style="color:#e2e8f0">Day 60:</strong> Full Assessment Complete<br/>
        🏛️ <strong style="color:#e2e8f0">Day 90:</strong> Official Decision Issued
      </div>
    </div>
...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    // Read session_id from query params or body
    ...
    if (!stripeRes.ok) {
      ...
      return new Response(JSON.stringify({ success: false, error: `Stripe error: ${err}` }), { status: 400, headers: CORS });
    }
    ...
    if (application_id) {
      try { app = await base44.asServiceRole.entities.Application.get(application_id); } catch (_) {}
    }
    ...
    if (app) {
      ...
      } catch (e: any) { console.warn("[peaVerifyPayment] App update failed:", e.message); }
    }

    // ── Upsert PaymentTransaction ───────────────────────────────────────────
    try {
      const existing = await base44.asServiceRole.entities.PaymentTransaction.filter({ stripe_session_id: session_id });
      if (!existing?.length) {
    ...
    } catch (e: any) { console.warn("[peaVerifyPayment] PaymentTransaction upsert failed:", e.message); }

    // ── Send emails ─────────────────────────────────────────────────────────
    if (apiKey && app?.applicant_email) {
      const html = buildPaymentConfirmEmail(app, session, year);
      try {
        await sendEmail(apiKey, app.applicant_email, `✅ Payment Confirmed — ${reference_code || app.reference_code} | Prime Endorsement Authority`, html);
      } catch (e: any) { console.error("[peaVerifyPayment] Applicant email failed:", e.message); }

      try {
        await sendEmail(
          apiKey,
          ADMIN_EMAIL,
          `💳 Payment Received — ${reference_code || app.reference_code} | £1,200.00`,
          `<p>Payment of <strong>£1,200.00</strong> received from <strong>${app.applicant_name}</strong> (${app.applicant_email}).<br/>
           Reference: <strong>${reference_code || app.reference_code}</strong><br/>
           Stripe Session: <code>${session_id}</code></p>`
        );
      } catch (e: any) { console.warn("[peaVerifyPayment] Admin email failed:", e.message); }
    }
    ...
    console.error("[peaVerifyPayment] Fatal:", err.message);
